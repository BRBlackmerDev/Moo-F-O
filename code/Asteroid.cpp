...
DrawAsteroid( RENDER_PASS * Pass, APP_STATE * AppState, CAMERA_STATE * Camera, ASTEROID Asteroid, flo32 Alpha = 1.0f, flo32 Scale = 1.0f ) {
    ...
    vec4 ColorA = TowardBlack( ASTEROID_DEFAULT_BASE_COLOR, 0.4f );
    if( Asteroid.Speed >= ASTEROID_HI_DAMAGE_SPEED_THRESHOLD ) {
        ColorA = TowardWhite( ASTEROID_DEFAULT_BASE_COLOR, 0.1f );
    }
...
internal void
PlayAsteroidCollisionAudio( APP_STATE * AppState, ASTEROID * Asteroid, ASTEROID * Collide ) {
    AUDIO_ID AudioID = AudioID_Asteroid_Collide_Large_01;
    if( Asteroid->Class <= AsteroidClass_Small ) {
        AudioID = AudioID_Asteroid_Collide_Small_01;
    }
    if( ( Collide ) && ( Collide->Class <= AsteroidClass_Small ) ) {
        AudioID = AudioID_Asteroid_Collide_Small_01;
    }
    ...
    flo32 Speed = Asteroid->Speed;
    if( Collide ) {
        Speed += Collide->Speed;
    }
    flo32 tSpeed = Clamp01( Speed / ( ASTEROID_HI_DAMAGE_SPEED_THRESHOLD * 1.5f ) );
    flo32 Volume = ASTEROID_COLLISION_VOLUME * lerp( 0.25f, tSpeed, 1.0f );
    if( Asteroid->Class == AsteroidClass_Tiny ) {
        Volume = 0.2f;
    }
    if( ( Collide ) && ( Collide->Class == AsteroidClass_Tiny ) ) {
        Volume = 0.2f;
    }
        ...
        for( int32 iAsteroid = 0; iAsteroid < AsteroidS->nAsteroid; iAsteroid++ ) {
            ...
            for( int32 iCollide = iAsteroid + 1; ( Asteroid->Health > 0 ) && ( iCollide < AsteroidS->nAsteroid ); iCollide++ ) {
                ...
                if( Collide->Health > 0 ) {
                    ...
                    if( DoCollide ) {
                        flo32 Collide_Radius   = ( Asteroid->Radius + Collide->Radius ) * 0.925f;
                        flo32 Collide_RadiusSq = Collide_Radius * Collide_Radius;
                        flo32 Collide_DistSq   = GetLengthSq( Asteroid->Position - Collide->Position );
                        
                        if( Collide_DistSq <= Collide_RadiusSq ) {
                            PlayAsteroidCollisionAudio( AppState, Asteroid, Collide );
            ...
            { // MINER
                if( Asteroid->Health > 0 ) {
                    ...
                    for( int32 iMiner = 0; iMiner < MinerS->nMiner; iMiner++ ) {
                        ...
                        if( DistSq <= RadiusSq ) {
                            PlayAsteroidCollisionAudio( AppState, Asteroid, 0 );
    ...
    { // Spawn New Asteroids
        ...
        for( int32 iAsteroid = 0; iAsteroid < nAsteroid; iAsteroid++ ) {
            ...
            if( ( Asteroid.Health == 0 ) && ( Asteroid.Class > AsteroidClass_Tiny ) && ( Asteroid.Flags & ASTEROID_FLAG__CAN_SPAWN_CHILDREN ) ) {
                ...
                if( Asteroid.ZeroPoint_IsActive ) {
                    vec2 DirFromPlayer = GetNormal( Asteroid.Position - Player->Position );
                    DirA = GetNormal( DirA + DirFromPlayer * 0.25f );
                }
